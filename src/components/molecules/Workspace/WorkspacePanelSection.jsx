    ...
    <div className='flex flex-col mt-2 px-2'>
      <div className='flex items-center gap-1 px-2 py-1 rounded-md group'>
        <Button
          ...
          variant='transparent'
          className='p-1 h-6 w-6 flex items-center justify-center text-slate-300'
        >
          {open ? <FaCaretDown className='size-4' /> : <FaCaretRight className='size-4' />}
        </Button>
        <button
          onClick={() => setOpen(!open)}
          className='flex-1 text-left text-sm font-medium text-slate-200 px-2 py-1 rounded hover:text-white transition-colors truncate'
        >
          {label}
        </button>
        {onIconClick && (
